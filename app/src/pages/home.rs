...
static ROADMAP_DATA: LazyLock<Vec<RoadmapStage>> = LazyLock::new(|| {
	vec![
		...
		RoadmapStage {
			...
			items: vec![
				StageItem { title: "Scrape Memes", description: "Gather large collection of memes from the internet along with their engagement metrics." },
				StageItem { title: "Data Science", description: "Design the calculations to create the numbers for betting and bet returns." },
		...
		RoadmapStage {
			...
			items: vec![
				StageItem { title: "Showdown Value", description: "Defined as the total returns of the best 5 of 7 meme bets in a hand." },
				StageItem { title: "Initial System", description: "Start by implement standard poker, then layer on the memes." },
			],
		},
	]
});
